...
def main():
    ...
    print("Disabling old execution jobs...")
    cur.execute("UPDATE jobs SET status='error', locked_by=NULL WHERE agendamento_id = 837 AND rotina='Execução' OR id IN (719, 720, 721)")
    conn.commit()

    print("Triggering new Execucao Job...")
    try:
        # Pede para executar o job (Isso não vai rodar a captura novamente pq a captura já teve success!)
        r = requests.post('http://localhost:8000/agendamentos/executar', json={'agendamento_id': 837})
